	...
	if(args != command) {
		*args = '\0';
		...
		if(strcmp(command, "info") == 0) {
			...
			cli_command_memory(command, args);
		}
...
static void cli_command_memory(const char *command, char *args)
{
	...
	token = args;
	if(cli_strip_word(&args)) {
		uart_printf("1 Incorrect format, [i2c|spi] [fill|dump [start address [length]]]\n");
		return;
	}
	action = token;
	...
	token = args;
	if(cli_strip_hex(&args)) {
		uart_printf("2 Incorrect format, [i2c|spi] [fill|dump [start address [length]]]\n");
		return;
	}
	...
	token = args;
	if(cli_strip_decimal_number(&args)) {
		uart_printf("3 Incorrect format, [i2c|spi] [fill|dump [start address [length]]]\n");
		return;
	}
	length = atoi(token);
	...
	if(strcmp(command, "i2c") == 0) {
		if(strcmp(action, "fill") == 0) {
			_24lc16b_write_bytes_addr(address, length);
		}
		else if(strcmp(action, "dump") == 0) {
			_24lc16b_read_bytes_hex_dump(address, length);
		}
		else {
			uart_printf("4 Incorrect format, [i2c|spi] [fill|dump [start address [length]]]\n");
		}
	}
	else if(strcmp(command, "spi") == 0) {
		if(strcmp(action, "fill") == 0) {
			_25lc040a_write_bytes_addr(address, length);
		}
		else if(strcmp(action, "dump") == 0) {
			_25lc040a_read_bytes_hex_dump(address, length);
		}
		else {
			uart_printf("5 Incorrect format, [i2c|spi] [fill|dump [start address [length]]]\n");
		}
	}
	else {
		uart_printf("6 Incorrect format, [i2c|spi] [fill|dump [start address [length]]]\n");
	}
}
